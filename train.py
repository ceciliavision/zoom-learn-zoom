...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    target_rgb=tf.placeholder(tf.float32,shape=[batch_size,None,None,3], name="target_rgb")
    ...
    loss_context, loss_ind=losses.compute_unalign_loss(out_rgb, target_rgb,
        tar_h=tar_h, tar_w=tar_w, tol=tol, losstype='l1', stride=stride)

    tf.summary.scalar('loss_context', loss_context)

    objDict['argmin'] = loss_ind
    lossDict = {}
...
if mode == "train":
    ...
    for epoch in range(1+epoch_offset, maxepoch+epoch_offset):
        ...
        for id in np.random.permutation(num_train):
            if input_raw_img[id] is None:
                ...
                fetch_list=[opt,merged,objDict,lossDict]
                st=time.time()
                _,out_summary,out_objDict,out_lossDict=sess.run(fetch_list,feed_dict=
                ...
                if cnt % save_summary_freq == 0:
                    writer.add_summary(out_summary, cnt+(epoch-1)*num_train)
    ...
    for id in np.random.permutation(num_test):
        ...
        print(target_rgb_img_orig.shape, input_rgb_img_orig.shape)
        ...
        gt_match = Image.fromarray(np.uint8(utils.apply_gamma(target_rgb_match)*255))
        ...
        gt_rgb_cropped = Image.fromarray(np.uint8(utils.apply_gamma(cropped_rgb[tol:tol+tar_h,tol:tol+tar_w,:])*255))
        input_rgb = Image.fromarray(np.uint8(utils.apply_gamma(input_rgb_img_orig)*255))
        input_rgb_cropped = Image.fromarray(np.uint8(utils.apply_gamma(cropped_raw_rgb)*255))
