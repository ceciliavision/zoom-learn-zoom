    ...
    for epoch in range(1,maxepoch):
        ...
        for id in np.random.permutation(num_train):
            if input_raw_img[id] is None:
                ...
                input_raw_img[id],target_rgb_img_orig = processed_dict['input_raw'], processed_dict['tar_rgb']
                print("Read in image shapes:",train_input_paths[id], input_raw_img[id].shape, target_rgb_img_orig.shape)
                if input_raw_img[id] is None or target_rgb_img_orig is None:
                ...
                row, col = input_raw_img[id].shape[1:3]
                target_rgb_img_orig, transformed_corner = utils.post_process_rgb(target_rgb_img_orig[0,...], (int(col*4),int(row*4)), processed_dict['tform'])
                target_rgb_img[id] = np.expand_dims(target_rgb_img_orig, 0)
                input_raw_img[id] = input_raw_img[id][transformed_corner['minw']:transformed_corner['maxw'],
