...
def conv2_2(batch_input, kernel=3, output_channel=64, stride=1, use_bias=True, scope='conv'):
    # kernel: An integer specifying the width and height of the 2D convolution window
    with tf.variable_scope(scope):
      pad_total = kernel - 1
      ...
      if np.random.rand() > 0.5:
        pad_beg = pad_total // 2
        pad_back = pad_total - pad_beg
      else:
        pad_back = pad_total // 2
        pad_beg = pad_total - pad_back
      
      batch_input = tf.pad(batch_input, [[0, 0], [pad_beg, pad_back], [pad_beg, pad_back], [0, 0]], "REFLECT")
      if use_bias:
          return slim.conv2d(batch_input, output_channel, [kernel, kernel], stride, 'VALID', data_format='NHWC',
...
# filter = tf.get_variable("filter", [4, 4, out_channels, in_channels], dtype=tf.float32, initializer=tf.xavier_initializer())
