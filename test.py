...
mode = 'train'
substr = 'rgb_x4_cont_coord_5_patch2_w5_camera_wb_flower' # 12 bits
task = 'restore_resnet'  # restore_srresnet_l1 , restore_resnet
save_root = '/export/vcl-nfs2/shared/xuanerzh/zoom/'
restore_path = save_root + 'rgb_x4_cont_coord_5_patch2_w5_camera_wb'
...
decay_step = 20000
decay_rate = 0.05
save_model_freq = 20000   # unit: step
save_img_freq = 5   #unit: step
save_summary_freq = 1000  #unit: step
...
def main():
    ...
    if mode == "train":
        ...
        def prepare_train_data(raw_path):
            ...
            if 'camera' not in task and is_gt_gamma == True:
            ...
            elif 'camera' not in task and 'tif' in task:
                rgb_path = os.path.dirname(raw_path).replace(subfolder, subfolder+'_process') + "/rawpng/" + \
        ...
        for id in range(num_test):
            ...
            rgb_camera_path = input_dict['src_path_raw'].replace(".ARW",".JPG")
            print("reference camera path: %s"%rgb_camera_path)
            rgb_camera =  np.array(Image.open(rgb_camera_path))
            cropped_input_rgb = utils.crop_fov(rgb_camera, 1./input_dict['ratio'])
            input_camera_rgb = Image.fromarray(cropped_input_rgb[raw_tol*2:cropped_raw_reshaped.shape[0]-raw_tol*2,
                raw_tol*2:cropped_raw_reshaped.shape[1]-raw_tol*2])
            ...
            input_camera_rgb_naive.save("%s/%s/%d_%d/input_rgb_camera_naive.png"%(task,test_folder,folderid,fileid))
            ...
            input_rgb_cropped.save("%s/%s/%d_%d/input_rgb_cropped_orig.png"%(task,test_folder,folderid,fileid))
            input_rgb_cropped = input_rgb_cropped.resize((int(input_rgb_cropped.width * up_ratio),
                        int(input_rgb_cropped.height * up_ratio)), Image.ANTIALIAS)
            if 'lum' in task:
            ...
            input_rgb.save("%s/%s/%d_%d/input_rgb.png"%(task,test_folder,folderid,fileid))
            input_rgb_cropped.save("%s/%s/%d_%d/input_rgb_cropped.png"%(task,test_folder,folderid,fileid))
            output_rgb.save("%s/%s/%d_%d/out_rgb.png"%(task,test_folder,folderid,fileid))
            gt_rgb.save("%s/%s/%d_%d/tar_rgb.png"%(task,test_folder,folderid,fileid))
            gt_rgb_cropped.save("%s/%s/%d_%d/tar_rgb_cropped.png"%(task,test_folder,folderid,fileid))
            gt_match.save("%s/%s/%d_%d/tar_rgb_match.png"%(task,test_folder,folderid,fileid))
    ...
    # Quick inference on a single image
        ...
        for up_ratio in up_ratio_list:
            ...
            if not os.path.isdir("%s/%s"%(task, inference_folder)):
                os.makedirs("%s/%s"%(task, inference_folder))
            inference_path = "/export/vcl-nfs2/shared/xuanerzh/zoom/test/dslr_10x_both/00434/00007.ARW"
            ...
            if patch_sz > 0:
                ...
                img_recons.save("%s/%s/%s/out_rgb.png"%(task,inference_folder,id))
            else:
                input_raw_img = np.expand_dims(input_raw_img_orig, 0)
                out_objDict=sess.run(objDict,feed_dict={input_raw:input_raw_img})
                output_rgb = Image.fromarray(np.uint8(utils.apply_gamma(utils.clipped(out_objDict["out_rgb"][0,...]),is_apply=False)*255))
                output_rgb = output_rgb.resize((int(output_rgb.width * resize_ratio),
                    int(output_rgb.height * resize_ratio)), Image.ANTIALIAS)
                output_rgb.save("%s/%s/%s/out_rgb_%d-%d.png"%(task,inference_folder,id,0,0))
            ...
            input_camera_rgb.save("%s/%s/%s/input_rgb_camera_orig.png"%(task,inference_folder,id))
            input_camera_rgb_naive = input_camera_rgb.resize((int(input_camera_rgb.width * up_ratio),
                int(input_camera_rgb.height * up_ratio)), Image.ANTIALIAS)
