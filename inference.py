...
def main():
    ...
    for id, inference_path in enumerate(inference_paths):
        ...
        for up_ratio in up_ratio_list:
            ...
            if not os.path.isdir("%s/%s/%s-%dx"%(task_folder, mode, up_ratio, id)):
                os.makedirs("%s/%s/%s-%dx"%(task_folder, mode, up_ratio, id))
            ...
            output_rgb.save("%s/%s/%s-%dx/out_rgb.png"%(task_folder,mode,up_ratio,id))

            input_camera_rgb = Image.fromarray(np.uint8(utils.clipped(cropped_input_rgb)*255))
            input_camera_rgb.save("%s/%s/%s-%dx/input_rgb_camera_orig.png"%(task_folder,mode,up_ratio,id))
            ...
            input_camera_rgb_naive.save("%s/%s/%s-%dx/input_rgb_camera_naive.png"%(task_folder,mode,up_ratio,id), compress_level=1)
